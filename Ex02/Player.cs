...
namespace Logic
{
    public class Player
    {
        ...
        private List<(int row, int column, object cardValue)> m_MemoryOfComputer;
        ...
        public void InitPlayer(string i_PlayerName, char i_TypeOfPlayer)
        {
            ...
            m_MemoryOfComputer = new List<(int, int, object)>(2);
        }
        ...
        public (int, int) GetKnownCard(object i_CardValue, Board i_Board)
        {
            (int row, int col) = (k_WrongIndexForComputer, k_WrongIndexForComputer);

            foreach (var item in m_MemoryOfComputer)
            {
                if (item.cardValue.Equals(i_CardValue) && i_Board.GetCardByIndex(item.row, item.column).IsCovered)
                {
